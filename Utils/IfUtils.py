...
def IsValidIpv6(ip):
    ...
    # If the IPv6 address is an IPv4 address (::ffff:x.x.x.x), then its good.
    if re.match("::ffff:((0|[1 - 9][0 - 9]{0 - 2})\.){3}(0|[1 - 9][0 - 9]{0 - 2})",
                strVal) is not None:
        return True
    ...
    # Ensure that there are too few or too many colons...
    if count <= 1 or count > 8:
        return False

    part = 0
    doubleColon = False
    # Check each part to make sure they are valid...
    for elem in ipElems:
        part = part + 1
        # If the size of the hexadecimal part is invalid..
        if len(elem) < 0 or 4 < len(elem):
            return False

        # If the part is empty but not on either end of the string...
        if 0 == len(elem) and 1 < part and part < count:
            # If we have seen the double colon already...
            if doubleColon:
                return False
            # Note we saw the double colon...
            doubleColon = True

        # If the contents are not valid hexadecimal...
        if re.match("!([0-9A-Fa-f]([0-9A-Fa-f]*))", elem) is not None:
            return False
    return True
