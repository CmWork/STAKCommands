...
def release_dhcp(device_handle_list):
    with AutoCommand("Dhcpv4ReleaseCommand") as dhcp_release_cmd:
        dhcp_release_cmd.SetCollection("BlockList", device_handle_list)
        dhcp_release_cmd.Execute()
    with AutoCommand("Dhcpv4ReleaseWaitCommand") as dhcp_cmd:
...
def run(TopologyConfig):
    ...
    try:
        ...
        if len(device_handle_list) > 0:
            release_dhcp(device_handle_list)
...
def _get_logger():
    return PLLogger.GetLogger("spirent.methodology.trafficcenter.ReleaseDhcpSubnetCommand")
