...
import manager.DeleteTestCaseCommand as DelCmd
...
def test_delete_validation(stc):
    ...
    assert res == ""
    res = DelCmd.validate(tc2.GetObjectHandle())
    assert res == ""
...
def test_delete_active_testcase(stc):
    plLogger = PLLogger.GetLogger('methodology')
    ...
    assert tc_obj
    tc_obj_path = tc_obj.Get("Path")
    assert os.path.exists(tc_obj_path)
    ...
    # Check the path
    assert not os.path.exists(tc_obj_path)
